...
class BPETokenizer:
    ...
    # 相邻token统计
    """
        tokens: 一个列表，包含当前文本被切分后的各个 bytes 片段。
        stats: 一个字典，用于累加统计所有成对 Token 出现的频率。
        """
    def _pair_stats(self, tokens, stats):
        for i in range(len(tokens) - 1):
            new_token = tokens[i] + tokens[i + 1]
            if new_token not in stats:#第三行：检查这个新组合是否已经在统计字典 stats 中
                stats[new_token] = 0#第四行：如果没出现过，在字典里为它开辟空间，初始计数设为 0
            stats[new_token] += 1#第五行：将这个组合的出现次数加 1
    ...
    def train(self, text_list, vocab_size):
        ...
        self.i2b = {v: k for k, v in self.b2i.items()}#反过来
...
if __name__ == '__main__':
    # 加载语料
    cn = open('大模型语料.txt', 'r',encoding='utf-8').read()


    # 训练
    tokenizer = BPETokenizer()
    tokenizer.train(text_list=[cn], vocab_size=12000)
    ...
    with open('tokenizer.bin', 'rb') as f:
        # 按照保存顺序读取数据
        b2i, sp_s2i, next_id = pickle.loads(f.read())

    # 构建一个易读的字典结构
    readable_vocab = {
        "info": {
            "total_vocab_size": next_id,
            "description": "BPE Tokenizer Vocabulary"
        },
        "special_tokens": sp_s2i,  # 特殊 Token 映射
        "vocabulary": []
    }

    # 遍历词表
    for byte_token, token_id in b2i.items():
        try:
            # 尝试解码为字符串（如汉字、英文） [cite: 1, 13]
            word = byte_token.decode('utf-8')
        except:
            # 如果是中间状态的字节碎片，转为十六进制显示
            word = f"hex:{byte_token.hex()}"

        readable_vocab["vocabulary"].append({
            "id": token_id,
            "token": word
        })
